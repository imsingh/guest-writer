...
**TL;DR:** In this article, you will learn how to develop an audio player app using **Angular** and **RxJS**. You will handle audio operations and state of application using RxJS. To secure your application, you will use **Auth0**. If needed, you can find the final code on this [GitHub repository](https://github.com/imsingh/auth0-audio/).
...
To handle media playback in a reactive way, you will adapt [JavaScript's `Audio` object](https://www.w3schools.com/jsref/dom_obj_audio.asp) with a RxJS *Observable* and you will also use the RxJS to manage the state of your audio player.
...
```
It will generate `player` component in `pages/player` directory.
...
This will generate a service in a file called `audio.service.ts` under `./src/services/audio/` and this service have to be added to `NgModule` in `app.module.ts`. Replace the contents of the `audio.service.ts` file with:
...
You are going to update `AudioService` to manage the state, since state is dependent on audio playback.
...
```
The `getFiles` method above basically mocks an HTTP request by returning an `Observable` with a hard coded `files` object.
...
* `redirectUri`: It's your URL where user will be redirect after login. You can use the same url as your callback url here.
* `logoutUrl`: It's your URL that you want your user to direct when he/she logout.

### Auth Service
After creating your Auth0 account and defining the auth0 config in environment file, you will need to define an authentication service in your Angular app. As such, create a new file called `auth.service.ts`in the same `./src/services/auth/` directory and add the following content to it:
...
* `logoutUrl`: it's the url where user is redirected after logout.
* `parseHash$`: it's an `Observable` which parse the hash and gives you back the auth result. 
* `checkSession$`: this `Observable` is use to check the session and then renew the token if required.
...
* `login()`: In login method, you authorize the user.
* `handleLoginCallback()`: After authentication, this method is going to be fired. It uses `parseHash$` Observable to parsh the auth result and then it sets the authentication state using `this._setAuth` method and finally redirect to `onAuthSuccessUrl`
* `_setAuth()`: this method takes `authResult` from parsed Auth data and initialize `token$`. It sets the Auth State in localstorage along with userProfile data.
* `authenticated()`: this getter method is used to check if user is authenticated or not using localstorage flag.
* `renewAuth()`: this method checks if user is authenticated or not and then check session if it's valid or not and set's auth state respectively.
* `logout()`: it logout the user by removing auth state and user data from localstorage. It also calls the `auth0.logout` method which redirect user to given `logoutUrl`.

> You have to add AuthService to the `providers` array of `NgModule`.

### Auth Guard

Now that you have created AuthService. You can create `AuthGuard`, which allows you to secure routes of the application. Use Angular Cli to generate the gaurd as follows:

```bash
ng generate guard guards/auth
```
This command will generate `auth.guard.ts` file under `/src/app/guards` directory. Update the content of file with the following:
...
It basically checks if user is logged in or not. If user is logged in, it allows user to access the given route otherwise redirect user to `/` route.
...
> You have to add this route to `router` config in `app-routing.module.ts` file. You also have to add `ProfileComponen`t to the `declaration` and `entryComponent` arrays of `NgModule`

#### Updating Player Component
In the last step, you have to update the player component by adding the Authentication UI and also link to Profile page. You are going to update `player.component.html` by adding the `mat-toolbar` in the header as follows:
