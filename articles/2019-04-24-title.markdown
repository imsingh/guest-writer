...
To handle media playback in a reactive way, you will adapt [JavaScript's `Audio` object](https://www.w3schools.com/jsref/dom_obj_audio.asp) with an RxJS *Observable* and you will also use the RxJS to manage the state of your audio player.
...
Having confirmed that you can run the basic app in the browser, you can start by installing the dependencies. To build your audio player, you will use the Angular Material. You can install it using `ng add` command:
...
Since you are going to use Angular Material Components, you need to import them inside your root `NgModules`. In order to do so, you are going to create a feature module named `MaterialModule` in the `material.module.ts` file under `./src/app` directory and add the following content:
...
Then, you are going to create the `app-player` component with @angular/cli:
...
Now that you understand why do you need an observable, you can start by creating service in your Angular app:
...
You are going to need a method to share the `statChange` BehaviorSubject to the rest of the application as follows:
...
In the last step, you have to update the player component by adding the Authentication UI and also link to the Profile page. You are going to update `player.component.html` by adding the `mat-toolbar` in the header as follows:
